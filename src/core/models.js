...
class Player {
	constructor(data) {
		...
		this.isConfirmed = this.status === "confirmed";
		this.isTentative = this.status === "tentative";
		this.isLate = this.status === "late";
		this.isBenched = this.status === "bench";
		this.isAbsence = this.status === "absence";
	...
	normalizeClass(className) {
		...
		// ⛔ NEVER treat attendance as a WoW class
	...
	determineRoles() {
		...
		if (this.spec) {
			...
			// Tank specs
			if (
				specLower.includes("protection") ||
				specLower.includes("guardian")
			) {
				roles.primary = "tank";
			}
			// Healer specs
			else if (
				specLower.includes("heal") ||
				specLower.includes("resto") ||
				specLower.includes("holy") ||
				specLower.includes("disc")
			) {
				roles.primary = "healer";
			}
			// DPS specs - be explicit about warrior DPS specs
			else if (
				specLower.includes("fury") ||
				specLower.includes("arms") ||
				specLower.includes("combat") ||
				specLower.includes("assassination") ||
				specLower.includes("subtlety") ||
				specLower.includes("enhancement") ||
				specLower.includes("elemental") ||
				specLower.includes("shadow") ||
				specLower.includes("balance") ||
				specLower.includes("feral") ||
				specLower.includes("retribution") ||
				specLower.includes("dps") ||
...
class Raid {
	...
	calculateComposition() {
		...
		this.players.forEach((player) => {
			...
			else if (player.roles.primary === "healer")
				this.composition.healers++;
			else if (player.roles.primary === "dps") this.composition.dps++;

			if (player.roles.all && player.roles.all.includes("utility"))
				this.composition.utility++;
		});
